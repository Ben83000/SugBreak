...
import { isAdmin, isAuthentificated } from "../auth/auth.js";
...
router.post("/", async (req, res) => {
  ...
  if (req.body.password) {
    ...
    if (userCreated) {
      await sendWelcomeEmail(lowerCaseEmail, confirmationToken);
    ...
    const userCreatedWithGoogle = await userModel.create(
      userToCreateWithGoogle
    );
...
router.get("/confirm/:token", async (req, res) => {
  ...
  if (user) {
    if (!user.isActive) {
      await userModel.updateOne({ email: decoded.email }, { isActive: true });
      res
        .status(302)
        .redirect(
...
router.patch("/:email", async (req, res) => {
  try {
    ...
    if (userUpdated) {
      ...
      res
        .status(404)
        .json({ message: "Impossible de mettre à jour les infos." });
    }
  } catch (error) {
    console.error(error);
