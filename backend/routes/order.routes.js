...
router.post("/add", async (req, res) => {
  ...
  const paymentMethodDecrypted = await getPaymentMethodDetails(
    data.paymentMethod
  ); // on décrypte le jeton pour recup les infos de paiement pour recuperer le .type (methode de paiement)
  const orderToCreate = {
    ...data,
    paymentMethod: paymentMethodDecrypted.type,
  };
  ...
  await sendConfirmationOrderEmail(
    ...
    orderCreated.owner,
    orderCreated.id
  ); // Envoi un mail de confirmation de commande à l'utilisateur
...
router.get("/user", isAuthentificated, async (req, res) => {
  try {
    ...
    if (orders) {
      ...
      res
        .status(200)
        .json({ message: "Vous n'avez pas encore commandé chez nous.." });
    }
  } catch (error) {
    res
      .status(500)
      .json({ message: "Problème serveur, veuillez réessayer.", error: error });
  }
});
