...
function ProductCard({ product }) {
  ...
  const { name, imageUrl, alt, description, category, _id, price, custom } =
    product;
  ...
  return (
    <section
      onClick={handleClick}
      className={cn(
        "select-none relative flex w-full min-w-72 h-36 text-slate-800 gap-1 p-2 bg-white shadow-custom-lg hover:shadow-custom-dark rounded-xl cursor-pointer overflow-hidden transition-transform duration-300 hover:scale-105"
      )}
    >
      <img
      ...
      <div className="flex flex-grow gap-1 h-full">
        <div className="h-full flex flex-col">
          ...
          <p className="text-wrap leading-[1.20rem] text-lg truncate-multiline">
            {description}
          </p>
          <p className="text-2xl text-amber-900 mt-auto">
            {formatPrice(price)}€
          </p>
        </div>
        <Button
