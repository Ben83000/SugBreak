...
function ModifyProductForm() {
  ...
  const handleDeleteImage = async (e) => {
    ...
    if (productToModify.imageName) {
      ...
      if (imageRef) {
        ...
        console.log(productToModify?.imageName);
  ...
  return productToModify !== null ? (
    <form
      ...
      className="
       hidden sm:grid grid-cols-4 grid-rows-4 max-w-[36rem] min-w-[36rem] p-4 max-h-full gap-2 select-none"
    >
      <div className="row-span-3 aspect-square rounded-lg overflow-hidden min-h-full max-h-full">
        {loading ? (
          ...
          <div
            className="min-h-full flex flex-col bg-pink-400/70 hover:bg-pink-400 cursor-pointer justify-center relative"
            onClick={handleClick}
          >
            <input
            ...
            <img
              src={
                ...
                `images/default/${productToModify?.category}.jpg`
              }
              alt="image uploaded"
      ...
      <Button type="submit" className="col-start-2  min-h-full ">
        Enregistrer les modif
      </Button>
      <Button
        type="button"
        onClick={handleDeleteButton} 
        className="min-h-full bg-red-600 hover:bg-red-700"
      >
    ...
    <h2 className="max-sm:hidden text-3xl ml-2 mt-2">
      Veuillez cliquer sur un produit pour le modifier/supprimer.
    </h2>
  );
}
